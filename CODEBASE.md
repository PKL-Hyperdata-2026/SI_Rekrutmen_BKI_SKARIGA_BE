...
- **`student_portfolios`**: Berkas lampiran portofolio/dokumen siswa dan alumni (`student_alumni_id`, `category_id`, `title`, `description`, `file_path`, `original_filename`). Menyimpan path unik berkas di storage publik (`file_path`) dan nama berkas asli saat diunggah (`original_filename`) untuk ditampilkan ke antarmuka pengguna. Mendukung audit trail (`created_by`, `updated_by`, `deleted_by`) dan `softDeletes`.
